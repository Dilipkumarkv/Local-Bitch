...
import com.dilipkumarkv.localgguf.engine.ChatTemplateHelper
import com.dilipkumarkv.localgguf.engine.GgufParser
...
class ExampleRobolectricTest {
    ...
    fun `test chat template formatting`() {
        val messages = listOf(
            com.dilipkumarkv.localgguf.data.model.MessageEntity(
                id = "m1",
                conversationId = "c1",
                role = com.dilipkumarkv.localgguf.data.model.MessageRole.USER,
                content = "Tell me a joke"
            )
        )
        val prompt = com.dilipkumarkv.localgguf.engine.ChatTemplateHelper.formatPrompt(messages, architecture = "qwen2")
    ...
    fun `test generation stats calculation`() {
        val stats = com.dilipkumarkv.localgguf.data.model.GenerationStats(
            promptTokens = 12,
            generatedTokens = 48,
            promptEvalMs = 120L,
            generationMs = 1600L,
    ...
    fun `test default generation parameters constraints`() {
        val params = com.dilipkumarkv.localgguf.data.model.GenerationParameters(
            temperature = 0.7f,
            maxTokens = 512,
            contextSize = 2048,
            topP = 0.9f,
    ...
    fun `test gbnf grammar presets and validation`() {
        val presets = com.dilipkumarkv.localgguf.engine.GbnfGrammarHelper.BUILT_IN_PRESETS
        ...
        presets.forEach { preset ->
            val validation = com.dilipkumarkv.localgguf.engine.GbnfGrammarHelper.validateGbnf(preset.gbnf)
        ...
        """.trimIndent()
        val convertedGbnf = com.dilipkumarkv.localgguf.engine.GbnfGrammarHelper.convertJsonSchemaToGbnf(testSchema)
        ...
        val choiceGbnf = com.dilipkumarkv.localgguf.engine.GbnfGrammarHelper.createChoiceEnumGbnf(choices)
