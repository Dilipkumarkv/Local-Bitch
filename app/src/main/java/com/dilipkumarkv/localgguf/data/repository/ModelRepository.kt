...
import com.dilipkumarkv.localgguf.data.db.ModelDao
            ...
            if (fileSize > 0 && usableSpace < fileSize + (50L * 1024 * 1024)) {
                return@withContext Result.failure(
                    IllegalStateException(
                        "Insufficient storage space on device. Required: ${com.dilipkumarkv.localgguf.engine.MemorySafetyHelper.formatBytes(fileSize)}, Available: ${com.dilipkumarkv.localgguf.engine.MemorySafetyHelper.formatBytes(usableSpace)}"
                    )
                )
            }
